...
from src.utilities.messages.exceptions.http.exc_details import (
    http_404_booking_details,
    http_404_email_details,
    http_404_field_details,
    http_404_id_details,
...
async def http_404_exc_field_not_found_request(id: int) -> Exception:
    return fastapi.HTTPException(
        status_code=fastapi.status.HTTP_404_NOT_FOUND,
        detail=http_404_field_details(id=id),
    )


async def http_404_exc_booking_not_found_request(id: int) -> Exception:
    return fastapi.HTTPException(
        status_code=fastapi.status.HTTP_404_NOT_FOUND,
        detail=http_404_booking_details(id=id),
    )
